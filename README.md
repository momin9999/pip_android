...
### プレイヤー
- **ジェスチャー操作**
  - 画面 **左側**を上下スワイプ → **明るさ** 調整（一番下まで下げると **Auto＝端末設定に追従**）
  - 画面 **右側**を上下スワイプ → **音量** 調整
  - 画面 **中央はジェスチャー無効** — ホームに戻るスワイプ等で誤って明るさ/音量が変わりません
  - 1本指 **ダブルタップ**（左/右）→ 10秒 戻し / 早送り
  - シングルタップ → コントロール表示/非表示
- **PIP（ピクチャインピクチャ）対応** — ボタン、またはホームに戻るだけで自動的に小窓再生へ（Android 12 以降は自動遷移、11 以下も対応）。小窓内に 再生/一時停止・10秒スキップ のボタン付き。**他アプリに切り替えても小窓の再生は継続**します。
- カスタムコントロール（シーク、10秒スキップ、前/次、画面ロック）。ロック中は誤操作でプレイヤーが閉じないよう戻る操作も無効化。
- 音声フォーカス制御・スリープ防止・バックグラウンド継続（**イヤホン/BT を外すと自動で一時停止**）。再生できないファイルは自動スキップ。

### ライブラリ（アプリ単体利用）
- **フォルダ選択**（SAF）— ストレージ権限不要で、内部ストレージ / SD カード / クラウドのフォルダを追加。
- **★ お気に入り** — 任意のフォルダ（ルート・サブフォルダ問わず）に星を付けると、**ホーム画面にその中身（サブフォルダ＆動画）を横スクロールの「棚」として展開**。フォルダを開かずに中の動画へ直接アクセスできます。
- フォルダを開くと配下の **サブフォルダ＆動画を一覧表示**（サムネイル・長さ・サイズ）。
